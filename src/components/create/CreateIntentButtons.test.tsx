...
describe('CreateIntentButtons', () => {
  it('renders the three output intents', () => {
    ...
    expect(screen.getByText('Clips from a video')).toBeInTheDocument();
    expect(screen.getByText('Content from a prompt')).toBeInTheDocument();
    expect(screen.getByText('Content from my KB')).toBeInTheDocument();
  });
  ...
  it('clip button opens the video flow', async () => {
    ...
    await userEvent.click(screen.getByText('Clips from a video'));
  ...
  it('prompt and KB buttons prefill the composer', async () => {
    ...
    await userEvent.click(screen.getByText('Content from a prompt'));
    expect(onPrefill).toHaveBeenCalledWith('Create content about ');
    await userEvent.click(screen.getByText('Content from my KB'));
