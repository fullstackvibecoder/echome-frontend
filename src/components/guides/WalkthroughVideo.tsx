...
interface WalkthroughVideoProps {
  /**
   * hero = larger heading, used on the guides index.
   * inline = used inside a guide page.
   * landing = no heading/copy, used on the marketing homepage where the
   * surrounding section supplies its own heading.
   */
  variant: 'hero' | 'inline' | 'landing';
}
...
export function WalkthroughVideo({ variant }: WalkthroughVideoProps) {
  ...
  return (
    <div id="watch" className={isLanding ? '' : 'mb-10'}>
      {!isLanding && (
        <>
          <h2
            className={
