        ...
        <div className="p-6">
          ...
          {(status === 'idle' || status === 'error') && (
            <>
              ...
              {/* YouTube: Connect Channel via OAuth */}
              {selectedPlatform === 'youtube' && (
                <div className="mb-6">
                  {(() => {
                    const googleAccount = connectedAccounts.find(a => a.platform === 'google');
                    const isConnected = googleAccount?.connected;
                    const isSyncing = syncStatus['google'] === 'syncing' || syncStatus['google'] === 'polling';
                        ...
                        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-sm font-medium text-emerald-700 dark:text-emerald-300">YouTube connected</p>
                                {googleAccount.username && <p className="text-xs text-emerald-600 dark:text-emerald-400">{googleAccount.username}</p>}
                              </div>
                            </div>
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-2xl hover:border-primary/50 hover:bg-gray-50 dark:hover:bg-gray-600 transition-all disabled:opacity-50"
                      >
                        ...
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">
                          {connectingPlatform === 'google' ? 'Connecting...' : 'Connect YouTube Channel'}
                        </span>
                      </button>
                    );
                  })()}
              ...
              {selectedPlatform && (
                <div className="mb-6">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    {selectedPlatform === 'youtube' ? 'Or paste a link manually' : `${PLATFORM_CONFIG[selectedPlatform].name} URL`}
                  </label>
                  <input
                    type="url"
                    value={url}
